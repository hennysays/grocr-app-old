...
exports.run = function(globalcallback) {
    var url = 'http://www.provigo.ca/en_CA/flyers.banner@PROV.storenum@8801.week@current.next@1.limit@500.html';
    var storeID = 1;
    request(url, function(err, resp, body) {
        ...
        expirydate = formatProvigoDate(date);
        ...
        $('.card-grid-layout .product').each(function() {
            ...
            if(details.indexOf('<br><br') != -1) {
                details = formatProvigoNameAndDescription(details);
                item.name = details[0];   
                item.description = '';
                if(details.length>1) {
                ...
                details = formatProvigoNameAndDescription(details);
                if(isFormatIrregularProvigoDetails(details)) {
                    item.name = title;
                    item.description='';
            ...
            price = formatProvigoPrices(price);
...
function formatProvigoNameAndDescription(details) {
...
function isFormatIrregularProvigoDetails(details) {
...
function formatProvigoPrices(price) {
...
function formatProvigoDate(date) {
